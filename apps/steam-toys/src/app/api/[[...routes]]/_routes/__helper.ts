...
app.get('/api/steam/player-stats/:id',async (c)=>{
  ...
  if (!userToken.valid && !siteOwnerToken.valid) throw new InvalidTokenError('Not Found Any Valid Token')
  ...
  const extractData = async (res: Response | null) => {
    ...
    if($('.login_modal').length > 0) {
      throw new InvalidTokenError('Not Found Access Token For Steam Community')
    }
